...
(function(global, undefined) {
	...
	//关键节点config defined load
	var mounts = {
		list: {},
		set: function(node,fn){
	...
	function define(id, deps, factory) {
		...
		if(id){
			...
			if (mod.status < STATUS.SAVED) {
				...
				//缓存到本地
				if (options.cache && id.charAt(0)!=='$') {
					try{
						define.cache(id, deps, factory);
					}catch(e){
						options.cache = false;
					}
		        }
	...
	//这里主要是开放给外部代码可以进行定制向修改
	define.cache = function(id, deps, factory){
        localStorage.setItem(coreConfig.prefix + id, deps.join(',')+'@@'+factory.toString());
	};
	//接收要加载模块的ID集合
