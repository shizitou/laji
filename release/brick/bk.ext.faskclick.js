 ...
 *		tapTimeout: 700ms 按压超时则取消点击行为：默认700ms
 * 		hover: false 是否开启对 data-kfc-hover 的监听
 * 		hoverDelay: 40ms hover延迟显示，默认40ms，其实这里在点击是也不太合理，
...
define('$KFC', function(require, exports, module) {
	...
	function FastClick(layer, options) { // new KFC(document. body || 指定范围,{})
		...
		// hover元素的时间控制器，在start的时候，要在tapTimeout以后关闭
		...
		// 是否开启hover功能：默认为false
		self.listenHover = !!options.hover;
		// self.hoverDelay = +options.hoverDelay === options.hoverDelay ? options.hoverDelay : 40;
		//绑定事件
		var events = [onMouse, onClick,
			onTouchStart, onTouchMove, onTouchEnd, onTouchCancel
		];
		for (var i = events.length - 1; i >= 0; i--) {
			self[events[i].name] = bind(events[i], self);
		}
		//是否监听 mouse 系列的事件
	...
	FastClick.prototype = {
		...
		updateScrollParent: function(targetElement) {
			...
			if (!scrollParent ||
				...
				//先检查当前元素
				if (hoverStart && !hoverElem && targetElement.dataset.kfcHover) {
					hoverElem = true;
				...
				do {
					//先解决hover的循环问题
					if (hoverStart && !hoverElem && parentElement.dataset.kfcHover) {
						hoverElem = true;
		...
		addHover: function(elem) {
			var self = this;
			if (!self.listenHover) return;
			elem = elem && elem.KFCH;
			if (!elem) return;
			var hoverClass = elem.dataset.kfcHover; //class
			// self.startHoverTimer = setTimeout(function(){
			elem.className += ' ' + hoverClass;
			// },self.hoverDelay);
			clearTimeout(self.hoverTimer);
			//为了防止意外发生，我们默认一定时间后，将hover移除
			self.hoverTimer = setTimeout(function() {
				self.removeHover(elem);
			}, self.tapTimeout);
		},
		removeHover: function(elem) {
			if (!this.listenHover) return;
			elem = elem && elem.KFCH;
			if (!elem) return;
			clearTimeout(this.startHoverTimer);
			var hoverReg = new RegExp(elem.dataset.kfcHover, 'g'); //class
			elem.className = elem.className.replace(hoverReg, '');
		},
	...
	function onTouchMove(event) {
		...
		if (this.targetElement === this.getElementFromTarget(event.target)) {
			...
			if (Math.abs(touch.pageX - this.touchStartX) > boundary ||
				...
				this.removeHover(this.targetElement);
				this.trackingClick = false;
	...
	function onTouchEnd(event) {
		...
		this.removeHover(targetElement);
		if (!this.trackingClick) {
			return true;
		}
