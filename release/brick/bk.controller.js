...
define('$controller', ['$config', '$template'], function(require, exports) {
    ...
    Controller.prototype = {
        ...
        //执行跳转
        execPage: function(curModule) {
            var curExports = curModule.exports,
                prevModule = this.runningPage,
                prevExports = prevModule ? prevModule.exports : null;
            //设置当前运行的模块
            this.runningPage = curModule;
            //解决当前显示中的模块离开
            if (prevModule) {
                this.prevPage = prevModule;
                //给这个模块添加 $to，跳转到哪里，在这之前，这个模块应该已经被添加 $from
                prevModule.sysParams.$to = curModule.pageId;
                curModule.sysParams.$from = prevModule.pageId;
                //这里的参数: 执行离开的模块的参数,当前显示的模块的参数 ,curModule.params
                BK.trigger('beforeLeave', [prevModule.params,prevModule.sysParams]);
                prevExports.leave && prevExports.leave(prevModule.params,prevModule.sysParams);
                BK.trigger('afterLeave', [prevModule.params,prevModule.sysParams]);
                //记录离开模块的页面滚动轴
                prevModule.saveScroll();
                //移除页面DOM元素
                if (prevModule.pageView){
                    prevModule.pageView.parentNode.removeChild(prevModule.pageView);
                    prevModule.viewIsShow = false;
                }
            }
            //先触发新的模块,将结构显示显示到页面中
            curModule.status > pageStatus.LOADED ?
                curModule.appendView() :
                curModule.init();

            //params: 给这个模块添加 $from， 从哪里来
            BK.trigger('beforeEnter', [curModule.params,curModule.sysParams]);
            curExports.enter && curExports.enter(curModule.params,curModule.sysParams);
            BK.trigger('afterLeave', [curModule.params,curModule.sysParams]);
            //前一个页面移除完毕后设置当前模块缓存住的y轴
            //如果此跳转是来自用户从页面点击进入的,则不设置y轴
            //如果是由浏览器或history引发的页面变更,则恢复y轴 
            curModule.options.fromUser ? curModule.setScroll(1) : curModule.setScroll(true) ;
            //释放内容
            curModule = curExports = prevModule = prevExports = null;
        },
        //加载页面模块
